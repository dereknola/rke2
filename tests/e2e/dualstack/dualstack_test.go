...
var _ = Describe("Verify DualStack Configuration", Ordered, func() {
	...
	It("Verifies ClusterIP Service", func() {
		_, err := e2e.DeployWorkload("dualstack_clusterip.yaml", tc.KubeconfigFile)
		Expect(err).NotTo(HaveOccurred())
	...
	It("Verifies Ingress", func() {
		_, err := e2e.DeployWorkload("dualstack_ingress.yaml", tc.KubeconfigFile)
	...
	It("Verifies NodePort Service", func() {
		_, err := e2e.DeployWorkload("dualstack_nodeport.yaml", tc.KubeconfigFile)
		Expect(err).NotTo(HaveOccurred())
	...
	It("Verifies podSelector Network Policy", func() {
		_, err := e2e.DeployWorkload("pod_client.yaml", tc.KubeconfigFile)
		Expect(err).NotTo(HaveOccurred())
		...
		_, err = e2e.DeployWorkload("netpol-fail.yaml", tc.KubeconfigFile)
		Expect(err).NotTo(HaveOccurred())
		cmd = "kubectl exec svc/client-curl --kubeconfig=" + tc.KubeconfigFile + " -- curl -m7 ds-clusterip-svc/name.html"
		_, err = e2e.RunCommand(cmd)
		Expect(err).To(HaveOccurred())
		_, err = e2e.DeployWorkload("netpol-work.yaml", tc.KubeconfigFile)
		Expect(err).NotTo(HaveOccurred())
		cmd = "kubectl exec svc/client-curl --kubeconfig=" + tc.KubeconfigFile + " -- curl -m7 ds-clusterip-svc/name.html"
		Eventually(func() (string, error) {
			return e2e.RunCommand(cmd)

...
var _ = Describe("Verify Basic Cluster Creation with Kine", Ordered, func() {
	...
	It("Verifies ClusterIP Service", func() {
		_, err := e2e.DeployWorkload("clusterip.yaml", tc.KubeconfigFile)
		Expect(err).NotTo(HaveOccurred())
	...
	It("Verifies NodePort Service", func() {
		_, err := e2e.DeployWorkload("nodeport.yaml", tc.KubeconfigFile)
		Expect(err).NotTo(HaveOccurred())
		for _, server := range tc.Servers {
	...
	It("Verifies LoadBalancer Service", func() {
		_, err := e2e.DeployWorkload("loadbalancer.yaml", tc.KubeconfigFile)
		Expect(err).NotTo(HaveOccurred())
	...
	It("Verifies Ingress", func() {
		_, err := e2e.DeployWorkload("ingress.yaml", tc.KubeconfigFile)
		Expect(err).NotTo(HaveOccurred())
		for _, server := range tc.Servers {
	...
	It("Verifies Daemonset", func() {
		_, err := e2e.DeployWorkload("daemonset.yaml", tc.KubeconfigFile)
		Expect(err).NotTo(HaveOccurred())
		nodes, err := e2e.ParseNodes(tc.KubeconfigFile, false)
		Expect(err).NotTo(HaveOccurred())
	...
	It("Verifies dns access", func() {
		_, err := e2e.DeployWorkload("dnsutils.yaml", tc.KubeconfigFile)
		Expect(err).NotTo(HaveOccurred())
	...
	It("Verify Local Path Provisioner storage ", func() {
		_, err := e2e.DeployWorkload("local-path-provisioner.yaml", tc.KubeconfigFile)
		Expect(err).NotTo(HaveOccurred())
